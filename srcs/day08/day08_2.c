...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/12/08 16:58:58 by TheRed            #+#    #+#             */
/*   Updated: 2024/12/08 16:58:58 by TheRed           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (antennas[++i].node)
	{
		...
		while (antennas[++j].node)
		{
			if (antennas[i].node == antennas[j].node)
			{
				...
				while (is_in_bound(split, new_pos[0]))
				{
				...
				while (is_in_bound(split, new_pos[1]))
				{
...
long int	resolve_part2(char *input, char **split)
{
	...
	for (int i = 0; map_copy[i]; i++)
		for (int j = 0; map_copy[j]; j++)
			if (map_copy[i][j] == '#' || ft_isalnum(map_copy[i][j]))
				result++;
